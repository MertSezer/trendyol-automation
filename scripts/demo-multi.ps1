...
function Prepare-Output {
  ...
  if (Test-Path ".\output\logs\multi-run.log") {
    Remove-Item ".\output\logs\multi-run.log" -Force
  }
...
function Test-Syntax {
  ...
  node -c .\tests\multi_product_fifo_test.js
  if ($LASTEXITCODE -ne 0) { throw "Syntax failed: multi_product_fifo_test.js" }

  Write-Host "Syntax OK" -ForegroundColor Green
}

function Run-Test {
  Write-Section "Running multi-product FIFO scenario"

  $env:TESTS = "./tests/multi_product_fifo_test.js"
  $env:CART_LIMIT = "3"

  npx codeceptjs run --steps 2>&1 |
    Tee-Object -FilePath ".\output\logs\multi-run.log"
  ...
  if ($exitCode -ne 0) {
    throw "Multi-product FIFO demo failed."
  }
...
switch ($Action) {
  ...
  default {
    ...
    Write-Host "  .\scripts\demo-multi.ps1 run"
    Write-Host "  .\scripts\demo-multi.ps1 open"
  }
}
